...
func Fetch_props(cid int, token string) error {
	...
	cnameUrl := fmt.Sprintf("https://open.taobao.com/handler/tools/getApiResult.json?isEn=false&env=2&format=json&apiName=taobao.itemcats.get&appKey=&cids=%d&fields=&parent_cid=&appSource=0&_tb_token_=%s", cid, token)
	resp, err := grequests.Get(cnameUrl, options)
	if err != nil {
	...
	if len(cnameData.ItemcatsGetResponse.ItemCats.ItemCat) == 0 {
	...
	file, err := os.Create(fmt.Sprintf("%d-%s.csv", cid, cnameData.ItemcatsGetResponse.ItemCats.ItemCat[0].Name))
	if err != nil {
		return errors.WithStack(err)
	}
	...
	for idx, prop := range propsResp.ItempropsGetResponse.ItemProps.ItemProp {
		...
		if idx == 0 {
			row = append(row, fmt.Sprintf("%d", cid), cnameData.ItemcatsGetResponse.ItemCats.ItemCat[0].Name)
		} else {
			row = append(row, "", "")
		}
